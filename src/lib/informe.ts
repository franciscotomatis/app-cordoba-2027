...
/**
 * Una fecha por mes para la tira de imágenes: se toma la más cercana al día 15
 * porque es la que mejor representa el mes; el primer o el último pase pueden
 * caer justo en una transición del cultivo.
 */
function fechasMensuales(serie: { fecha: string }[]) {
  ...
  for (const p of serie) {
    ...
    if (!actual) {
      porMes.set(mes, p.fecha);
      continue;
    }
    const dia = (f: string) => Math.abs(Number(f.slice(8, 10)) - 15);
    if (dia(p.fecha) < dia(actual)) porMes.set(mes, p.fecha);
  }
  return [...porMes.entries()].sort((a, b) => a[0].localeCompare(b[0]));
}
...
export async function generarInforme(
  ...
  for (let i = 0; i < lotes.length; i++) {
    ...
    if (clima?.resumenTemperatura) {
      y += 13;
      doc.text(
        `Período analizado: ${fechaCorta(clima.resumenTemperatura.desde)} → ${fechaCorta(clima.resumenTemperatura.hasta)}`,
        MARGEN,
        y
      );
    }
    ...
    for (let m = 0; m < meses.length; m++) {
      ...
      try {
        const r = await fetch(`/api/lotes/${l.lote_id}/ndvi/imagen?fecha=${fecha}`);
        if (!r.ok) throw new Error("sin imagen");
        doc.addImage(await comoDataUrl(await r.blob()), "PNG", x, yImg, ladoImagen, ladoImagen);
      } catch {
        doc.setDrawColor(216, 210, 200);
        doc.setLineWidth(0.5);
        doc.rect(x, yImg, ladoImagen, ladoImagen);
        doc.setFontSize(7);
        doc.text("sin imagen", x + 4, yImg + 12);
      }
    }
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(...tenue);
    doc.text(ndvi.fuente, MARGEN, y + filas * (ladoImagen + 24) + 6);
  }
